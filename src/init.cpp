...
void Shutdown()
{
    ...
    RenameThread("CryptoSpots-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: CryptoSpots.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: CryptoSpotsd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 33222 or testnet: 33220)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 33223 or testnet: 33221)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCryptoSpotsamount=<n> " + _("Keep N CryptoSpots anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CryptoSpots is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CryptoSpots is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CryptoSpots version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "CryptoSpots server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CryptoSpots") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CryptoSpots to complete") << "\n";
    ...
    nAnonymizeCryptoSpotsAmount = GetArg("-anonymizeCryptoSpotsamount", 0);
    if(nAnonymizeCryptoSpotsAmount > 999999) nAnonymizeCryptoSpotsAmount = 999999;
    if(nAnonymizeCryptoSpotsAmount < 2) nAnonymizeCryptoSpotsAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeCryptoSpotsAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize CryptoSpots Amount %d\n", nAnonymizeCryptoSpotsAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CryptoSpots+1000 == (.1CryptoSpots+100)*10
       10CryptoSpots+10000 == (1CryptoSpots+1000)*10
    */
