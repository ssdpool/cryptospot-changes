...
int nAnonymizeCryptoSpotsAmount = 500;
...
#else
    const char* pszModule = "CryptoSpots";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CryptoSpots
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CryptoSpots
    // Mac: ~/Library/Application Support/CryptoSpots
    // Unix: ~/.CryptoSpots
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptoSpots";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CryptoSpots.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "CryptoSpotsd.pid"));
