...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("CryptoSpots"))
        return false;
...
{
    // Convert CryptoSpots:// to CryptoSpots:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("CryptoSpots://"))
    {
        uri.replace(0, 12, "CryptoSpots:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoSpots.lnk";
}
...
{
    return GetAutostartDir() / "CryptoSpots.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CryptoSpots-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  CryptoSpots-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CryptoSpots-Qt"));
